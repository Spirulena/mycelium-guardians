...
@onready var _lc: LevelController
@onready var _view: LevelView
@onready var _gui: GUIController

func _init() -> void:
	pass

func setup(level_controller: LevelController, view: LevelView, gui: GUIController):
	_lc = level_controller
	_view = view
	_gui = gui
	name = "MainCameraComponent"
...
func _ready():
	pass
	# TODO: pass differently then with Events
	#Events.change_move_with_keys_state.connect(_on_change_move_with_keys_state)
...
func handle_mouse_drag2(event_relative):
	mouse_drag = true
	new_position -= event_relative * dragSensitivity / zoom

func handle_mouse_drag(event_relative):
	mouse_drag = true
	# Calculate the target position based on drag but do not immediately set it
	var drag_target_position = position - event_relative * dragSensitivity / zoom
...
func _process(delta):
	...
	if is_auto_moving:
		...
		if position.distance_to(target_position) < CLOSE_ENOUGH:
			# If close enough, stop auto moving
			is_auto_moving = false
		else:
			position = position.lerp(target_position, 1.0 - exp(-delta * 5))
	else:
