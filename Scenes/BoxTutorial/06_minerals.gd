class_name TutorialMinerals extends LevelView
...
func _ready() -> void:
	...
	absorber_label.text = "Grow Smog Absorbers: %d/%d" % [absorber_amount, absorber_target]
...
func exit_instructions():
	tutorial_replay_component.hide_replay_window()
	tutorial_replay_component.setup(exit_summary, exit_summary)
...
func  _on_view_changed(change: Dictionary):
	...
	if change.type == GUIController.Actions.GrowStructure and change.state == 'place_request':
		if change.building_id == BuildingObject.StructureType.Storage_Minerals:
		...
		elif change.building_id == BuildingObject.StructureType.Absorber_Smog:
			if absorber_amount >= absorber_target:
				pass_change = false
		...
		if change.prev == null:
			...
			if id == BuildingObject.StructureType.Absorber_Smog:
				...
				if absorber_amount >= absorber_target:
					...
					# till in the minerals
					var limit = level_controller.get_tally_storage_limit(ResourceObject.ResourceType.Minerals)
					level_controller.level_data.set_tally(ResourceObject.ResourceType.Minerals, limit)
