...
func update_one_tile(tile: TileObject):
	...
	var value = clamp(_lc.get_water_level(coords), 0, 1)
...
func show_overlay():
	...
	tween_a.tween_property(self, "modulate:a", 1.0, 0.4).from(0.0).set_trans(Tween.TRANS_CIRC).set_ease(Tween.EASE_IN_OUT)
