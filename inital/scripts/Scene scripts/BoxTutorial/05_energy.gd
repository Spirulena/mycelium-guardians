class_name TutorialEnergy extends LevelView
...
func _ready() -> void:
	absorbers = []
	buildable = {
		BuildingObject.StructureCategory.Absorber: [BuildingObject.StructureType.Absorber_Radiation],
	}
	...
	absorber_label.text = "Grow Radiation Absorbers: %d/%d" % [absorber_amount, absorber_target]
	storage_label.text = "Grow Energy Storage Collector: %d/%d" % [storage_amount, storage_target]
...
func exit_instructions():
	tutorial_replay_component.hide_replay_window()
	tutorial_replay_component.setup(finish_tutorial, finish_tutorial)
...
func  _on_view_changed(change: Dictionary):
	pass_change = true
	# TODO: filter
	# maybe use component filter for buildings
		#parent.view_changed({
	...
	if change.type == GUIController.Actions.GrowStructure and change.state == 'place_request':
		if change.building_id == BuildingObject.StructureType.Storage_Energy:
			if absorber_amount < absorber_target:
				...
				print_debug("Grow absorber first")
				pass_change = false
			#var curr_action = GUIController.Actions.GrowStructure
			#game_ui_controller.set_selected(null)
			#game_ui_controller._mouse_controller._state_handlers[curr_action].reset()
		# Then change if both absorbers are already built
			elif not absorbers_done:
				print_debug("wait for absorbers to be grown")
				pass_change = false
			...
			#elif storage_amount >= storage_target:
				#print_debug("Enought storage grown")
				#pass_change = false
				#game_ui_controller.view_changed.emit.call_deferred({
					#'type': GUIController.Actions.GrowStructure,
					#'state': 'place_not_allowed',
		...
		elif change.building_id == BuildingObject.StructureType.Absorber_Radiation:
			if absorber_amount >= absorber_target:
				pass_change = false
...
func _on_model_changed(change: Dictionary):
	# need to capture view, the placed thing, or even before ?
	# selected ?
	# then block, placing ?
	# or learn to rebuild build menu.
	#if change.type == ModelObject.Type.Building:
		#if change.prev == null:
			#var model: BuildingObject = change.curr
			#var id = model.get_building_type()
			#if id == BuildingObject.StructureType.Storage_Energy and absorber_amount < absorber_target:
				#pass_change = false
				## Communicate
			#elif id == BuildingObject.StructureType.Absorber_Radiation:
				#if absorber_amount >= absorber_target:
					#pass_change = false
			#else:
				#pass_change = true
	...
	if change.type == ModelObject.Type.Building:
		if change.prev == null:
			...
			var model: BuildingObject = change.curr
			var id = model.get_building_type()
			if id == BuildingObject.StructureType.Absorber_Radiation:
				absorber_amount += 1
				# do the scale tweens
