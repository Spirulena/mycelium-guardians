class_name WaterStorageOverlayPresenter extends Node2D
...
func _ready():
	resource_type = ResourceObject.ResourceType.Water
	_init_tile_data()
...
func show_overlay():
	...
	tween_a.tween_property(self, "modulate:a", 1.0, 0.2).from(0.0).set_trans(Tween.TRANS_CIRC).set_ease(Tween.EASE_IN_OUT)
